...
public class VisualizerView extends View implements Visualizer.OnDataCaptureListener {
    ...
    protected static byte[] mData = new byte[CYLINDER_NUM];//音量柱 数组
    private static int[] data = new int[CYLINDER_NUM];
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        xr = width / (float) DN_W;
        yr = height / (float) DN_H;

        strokeWidth = DN_SW * yr;
        strokeLength = DN_SL * xr;
        hgap = (int) ((width - strokeLength * CYLINDER_NUM) / (CYLINDER_NUM + 1));
        vgap = (int) (height / (MAX_LEVEL + 2));//频谱块高度
    ...
    protected void drawCylinder(Canvas canvas, float x, byte value) {
        ...
        for (int i = 0; i < value; i++) { //每个能量柱绘制value个能量块
            ...
            //绘制频谱块
            mPaint.setColor(Color.GREEN);//画笔颜色
            canvas.drawLine(x, y, (x + strokeLength)-15, y, mPaint);//绘制频谱块

            //绘制音量柱倒影
            if (i <= 6 && value > 0) {
                mPaint.setColor(Color.GREEN);//画笔颜色
                mPaint.setAlpha(100 - (100 / 6 * i));//倒影颜色
                canvas.drawLine(x, y1, (x + strokeLength)-15, y1, mPaint);//绘制频谱块
            }
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < CYLINDER_NUM / 2 - 4; i++) { //绘制25个能量柱
            ...
            //             drawCylinder(canvas, strokeWidth / 2 + hgap + i * (hgap + strokeLength), data[i]);
        ...
        for (int i = CYLINDER_NUM; i >= CYLINDER_NUM / 2 - 4; i--) {
            ...
            //             drawCylinder(canvas, strokeWidth / 2 + hgap + (CYLINDER_NUM / 2 + j - 1) * (hgap + strokeLength), data[i - 1]);
        }
